...
def ziskaj_vstup(pokus):
    ...
    if len(vstup) != 4 or not vstup.isnumeric():
        ...
        return ziskaj_vstup.tail_call(pokus)
...
def hraj():
    ...
    kod = vygeneruj_kod()
    return herna_slucka.tail_call(kod, 1)
...
def herna_slucka(kod, pokus):
    if pokus == 11:
        print("GAME OVER!\n\n"
                ...
                f"Správny kód bol {''.join(map(str, kod))}.\n")
        return hraj.tail_call() if hrat_znovu() else None

    vstup = ziskaj_vstup(pokus)

    if vstup == kod:
        print(f"Uhádli ste! Tajný kód je {''.join(map(str, kod))}.\n")
        return hraj.tail_call() if hrat_znovu() else None
    ...
    print(''.join(vystup), "\n")
    # print(len(traceback.format_stack()))
